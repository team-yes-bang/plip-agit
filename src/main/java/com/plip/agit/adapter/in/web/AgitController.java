...
public class AgitController {
	...
	@Operation(
			summary = "아지트 생성",
			description = "아지트와 방장 프로필을 함께 생성하고 초대 코드를 발급합니다. 액터는 Gateway가 주입한 X-User-UUID에서 식별합니다."
	)
	@PostMapping
	...
	@Operation(
			...
			description = "접속 유저가 ACTIVE로 속한 아지트 목록(제목·UUID)을 반환합니다. "
					...
					+ "이후 최근 토픽순으로 교체 예정입니다. 액터는 Gateway가 주입한 X-User-UUID에서 식별합니다."
	)
	@GetMapping("/me")
	...
	@Operation(
			summary = "아지트 입장",
			description = "초대 코드로 아지트에 입장합니다. 신규는 GUEST 프로필을 생성하고, LEFT는 닉네임·이미지를 갱신한 뒤 ACTIVE로 전환합니다. 이미 ACTIVE면 409, BANNED면 403, 정원 초과면 409를 반환합니다. 액터는 Gateway가 주입한 X-User-UUID에서 식별합니다."
	)
	...
	@Operation(
			summary = "아지트 정보 변경",
			description = "아지트장이 제목·인원·소개글·섬네일을 수정합니다. 변경 인원수는 현재 ACTIVE 인원보다 작을 수 없으며, 그 외 검증은 생성과 동일합니다. 액터는 Gateway가 주입한 X-User-UUID에서 식별합니다."
	)
	...
	@Operation(
			...
			description = "접속 유저가 해당 아지트에서 ACTIVE인 본인 닉네임·프로필 이미지를 부분 수정합니다. "
					+ "미전달 필드는 유지합니다. 액터는 Gateway가 주입한 X-User-UUID에서 식별합니다."
	)
	...
	@Operation(
			summary = "아지트장 위임",
			description = "아지트장이 ampId의 ACTIVE 게스트에게 방장 권한을 위임합니다. 성공 시 204를 반환합니다. 액터는 Gateway가 주입한 X-User-UUID에서 식별합니다."
	)
	...
	@Operation(
			summary = "초대 코드 재발급",
			description = "아지트장이 초대 코드를 재발급합니다. 호출마다 새 코드를 반환합니다. 연타 방지는 FE에서 처리합니다. 액터는 Gateway가 주입한 X-User-UUID에서 식별합니다."
	)
	...
	@Operation(
			summary = "아지트에서 내보내기",
			description = "아지트장이 ampId로 멤버를 내보냅니다. status를 BANNED로 바꾸고 bans 이력을 남깁니다. ACTIVE HOST는 내보낼 수 없습니다. 이미 BANNED인 경우 변경 없이 성공합니다. 액터는 Gateway가 주입한 X-User-UUID에서 식별합니다."
	)
	...
	@Operation(
			summary = "아지트 나가기",
			description = "본인이 아지트에서 나갑니다. GUEST는 status를 LEFT로 변경합니다. HOST는 ACTIVE 인원이 본인뿐일 때만 나갈 수 있으며, 이때 아지트는 소프트 삭제됩니다. 이미 LEFT이거나 BANNED인 경우 변경 없이 성공합니다. 액터는 Gateway가 주입한 X-User-UUID에서 식별합니다."
	)
	...
	@Operation(
			summary = "아지트 밴 해제",
			description = "아지트장이 userUuid 기준으로 밴을 해제합니다. BANNED인 멤버는 status를 LEFT로 바꾸고 bans 이력에 해제 시각을 기록합니다. 이미 LEFT인 경우 변경 없이 성공합니다. 액터는 Gateway가 주입한 X-User-UUID에서 식별합니다."
	)
